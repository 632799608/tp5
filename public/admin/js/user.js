  layui.use(['element','laypage','layer','form'], function(){
      ...
      vn = new Vue({
          ...
          methods: {
              list: function() {
                  var _this = this;
                  axios.post('/admin/user/index', this.search).then(function(response) {
                      if (_this.pages != response.data.last_page) {
                          _this.$set(_this, 'pages', response.data.last_page);
                          _this.page();
                      }
                      _this.$set(_this, 'user', response.data.data);
              ...
              addHtml: function() {
                  layer.open({
                    ...
                    type: 2, 
                    scrollbar:false,
                    content: '/admin/user/create', //这里content是一个普通的String
                    area:['800px','500px']
                  });
              },
              editHtml: function(id) {
                  layer.open({
                    title:'管理员编辑',
                    type: 2, 
                    content: '/admin/user/edit/id/'+id, //这里content是一个普通的String
                    area:['800px','500px']
                  });
              },
              allDel: function() {
              ...
              elDel: function(id) {
                  var _this = this;
                  layer.confirm('确认删除管理员？', function(index) {
