  layui.use(['element','laypage','layer','form'], function(){
      ...
      vn = new Vue({
          ...
          methods: {
              list: function() {
                  var _this = this;
                  axios.post('/admin/role/index', this.search).then(function(response) {
                      if (_this.pages != response.data.last_page) {
                          _this.$set(_this, 'pages', response.data.last_page);
                          _this.page();
                      }
                      _this.$set(_this, 'role', response.data.data);
              ...
              addHtml: function() {
                  layer.open({
                    ...
                    type: 2, 
                    scrollbar:false,
                    content: '/admin/role/create', //这里content是一个普通的String
                    area:['80%','90%']
                  });
              },
              editHtml: function(id) {
                  layer.open({
                    title:'角色编辑',
                    type: 2, 
                    content: '/admin/role/edit/id/'+id, //这里content是一个普通的String
                    area:['80%','90%']
                  });
              },
              allDel: function() {
              ...
              elDel: function(id) {
                  var _this = this;
                  layer.confirm('确认删除角色？', function(index) {
                      axios.post('/admin/role/delete',{id:id}).then(function(response) {
